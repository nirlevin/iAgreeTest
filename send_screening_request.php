...
//MY						$partiesArray[] = array ('email' => $landlord['email'], 'role' => 'landlord');

//MY							$partiesArray[] = array ('email' => $contract['my_email'], 'role' => 'landlord');
					}
				}
...
/*MY 			/*Remove any party that is no longer needed from the db *
...
//MY					$result = updateParty($contractId, $contract['my_email'], $creatorData);

					$result = true;//MY
					
					if($result == true) {
						
//MY						/* Prepare a list of emails of all recipients */
						foreach($partiesArray as $party) {
							$partiesEmails[] = $party['email'];
						}
						
						/* Send the contract to all parties while identified as the logged-in user */
						$mailSent = sendTenantsScreeningRequest($user, $contract, $partiesEmails);
