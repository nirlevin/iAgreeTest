<?php
	...
	/**********************************/
		...
		//	scanned_id upoad function
	
	if(isset($_FILES['my_id_img'])){
		$imgLoadOk = 1;
	...
	if (isset($_POST['form-submitted-reg'])) {
        ...
        $first_name = $_POST['my_first_name']; //else, assign it a variable
		$last_name = $_POST['my_last_name'];
		$identity_number = $_POST['my_identity_number'];
		$scanned_id = $_SESSION['temp_id_img'];
        ...
        if (doesExist($email) == false) { //if no previous user is using this email
			...
			$userId = addUser($first_name, $first_name, $last_name, $email, $password, $identity_number, $scanned_id, $activation);
            ...
            if ($userId > 0) {
				...
				//MY Activation code override
				$activationUrlMY = 'site-login/activateovrd.php?email=' . urlencode($email) . "&key=$activation&location=$login_redirect";
				$activationUrlTenant = 'site-login/activateovrd.php?email=' . urlencode($email) . "&key=$activation&location=index.php";
			
				//send the confirmation e-mail
				$mailSent = sendRegistrationConfirmationEmail($email, $first_name, $activationUrl);
				...
				if($mailSent) {
					...
					if (isset($_GET['rid'])){
						...
						//under the assumption that register with "rid" can only be tenant
						
						$contractId = getContractIdByRandomId($_GET['rid']);
						$contractTemp = getContractById($contractId);
						
						if ($contractTemp){
							
							$tenantData = array (
							'screen_request_approved' => 1,
							'screening_complete' => 1
							);
						
						}
						
						$result = 0;			
						$result = updateParty($contractId, $email, $tenantData);
						
					if($result){$registerSuccessAlert = 2;}	
...
<div class="registerBlock">
	...
	<div class="registerInner">
    	...
    	<div class="registerBox" id="registerSuccessAlert">
            ...
            <a href="<?=$activationUrlMY?>">אשר</a>
        
        </div><!--registerBox-->
        
        <div class="registerBox" id="registerSuccessAlert2">
        
        	<img src="img/IAgreeLogoSmall.png">
            <h1>נרשמת לאתר iAgree בהצלחה!</h1>
            <h2>הינך מוזמן להיכנס לאתר באמצעות הדוא"ל והסיסמה<br>ע"מ לעדכן את הפרופיל שלך ולהפוך אותך לשוכר אטרקטיבי יותר.</h2>
            <h2>במידה ובעל הדירה יבחר בך כשוכר - תקבל הודעה נוספת בדוא"ל.</h2>
            
            <a href="<?=$activationUrlTenant?>">אשר</a>
        ...
        <form id="loginForm" method="post">
            ...
            <div class="loginLeft">
            	...
            	<div class="rightLineBlock">
                    ...
                    <div class="contractInput" id="load_id" style="margin:0;" >
...
function idImgClick(){
	...
	$("input:file").change(function (){	
		...
		$('.load_id').css('background-color','rgba(0,204,102,1)');
...
function alerts(){
	...
	<? if($registerSuccessAlert == 2){ ?> 
		
		$('.registerBlock').css('display','table');
		$('#registerSuccessAlert2').fadeIn();
...
$(document).ready(function() {
	...
	<?php
		...
		if((isset($user)) or ($_SESSION['temp_id_img'] != "")){
